...
def get_latest_ndvi():
    ...
    # ✅ Ensure correct column name (`rast` instead of `ndvi_raster`)
    cur.execute("SELECT ST_AsTIFF(ndvi_raster) FROM ndvi_data ORDER BY rid DESC LIMIT 1;")
    result = cur.fetchone()
...
def get_ndvi_info():
    ...
    # ✅ Corrected SQL: Convert raster to geometry before using ST_XMin/ST_YMin
    cur.execute("""
        SELECT 
               ST_XMin(ST_ConvexHull(ndvi_raster))::float, 
               ST_YMin(ST_ConvexHull(ndvi_raster))::float, 
               ST_XMax(ST_ConvexHull(ndvi_raster))::float, 
               ST_YMax(ST_ConvexHull(ndvi_raster))::float
        FROM ndvi_data
        ORDER BY rid DESC
        LIMIT 1;
    """)
...
def get_ndvi_classified():
    ...
    cur.execute("SELECT ST_AsGDALRaster(ndvi_raster, 'GTiff') FROM ndvi_data ORDER BY rid DESC LIMIT 1;")
    result = cur.fetchone()
    ...
    # Classify NDVI into growth stages
    ...
    out_raster = BytesIO()
    with rasterio.open(out_raster, 'w', driver='GTiff', height=dataset.height, width=dataset.width,
                       count=1, dtype=rasterio.uint8, crs=dataset.crs, transform=dataset.transform) as dst:
